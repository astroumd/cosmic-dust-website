...
{
    //This is a temporary fix. The mat check can be removed entirely
    int i, material_check = 1, material_check_2 = 1;
    
    char command[MAX_LEN], color_1[MAX_LEN], color_in_full[MAX_LEN], color_2[MAX_LEN], material_1[MAX_LEN], material_2[MAX_LEN];
    char *original_path = "./backend_database";
    ...
    float precision = 0, brightness_1 = 0, brightness1_1 = 0, brightness1_2 = 0, brightness2_1 = 0, brightness2_2 = 0;
    ...
    if (argc == 5)
    {
        ...
        if (!strcmp(command, "find_color"))
        {
            ...
            color = -2.5 * log10((brightness_1 / brightness_2));
    ...
    else if (argc == 8)
    {
        ...
        else if (!strcmp(command, "find_color"))
        {
            sscanf(argv[2]," %s", material_1);
            ...
            //find polarization assuming we are using the blue filter.
            strcpy(color_1, "B");
            result_1 = find_polarization(fp, &polarization_1, &brightness1_1, material_1, color_1, size_1, angle, original_path);
            //find polarization assuming we are using the red filter.
            strcpy(color_2, "R");
            result_1 = find_polarization(fp, &polarization_1, &brightness1_2, material_1, color_2, size_1, angle, original_path);
            
            //find polarization assuming we are using the blue filter.
            strcpy(color_1, "B");
            result_2 = find_polarization(fp, &polarization_1, &brightness2_1, material_2, color_1, size_2, angle, original_path);
            //find polarization assuming we are using the red filter.
            strcpy(color_2, "R");
            result_2 = find_polarization(fp, &polarization_1, &brightness2_2, material_2, color_2, size_2, angle, original_path);            
            ...
            fprintf(fp, "Angle, Step size\n %d, %f", angle, step);
            fprintf(fp, "\n\n Proportion of %s, Proportion of %s, Color\n", material_1, material_2);
                for (j = 0; j < 1; j+=step)
                {
                    //Given the color of both materials
                    color = -2.5* log10((brightness1_1 * j + brightness2_1 * (1-j))/ (brightness1_2 * j + brightness2_2 * (1-j)) );
                    fprintf(fp, "%.0f, %.0f, %f\n", (float)(j * 100),(float)((1-j) * 100), color);
                }
        }
    ...
    else if (argc == 9)
    {
        ...
        if (!strcmp(command, "find_pola") || !strcmp(command, "find_bright"))
        {
            ...
            {
                fprintf(fp, "\n\n Proportion of %s, Proportion of %s, Polarization\n", material_1, material_2);
                for (j = 0; j < 1; j+=step)
                {
                    ...
                    if (s11 != 0)
                    {
                        ...
                        fprintf(fp, "%.0f, %.0f, %f\n", (float)(j * 100),(float)((1-j) * 100), total_polarization);
                    }
                }
            }
            if (!result_3)
...
int find_size_r_two_helper_2(FILE * fp, char* material_1, char* material_2, char* original_path, float polarization, float pola_range, float color, float color_range, int angle, float precision, float** array_file_1_red, float** array_file_1_blue, float** array_file_2_red, float** array_file_2_blue)
{
    ...
    for (int i = 0; i < SIZES_LEN; i++)
    {
        for (int j = 0; j < SIZES_LEN; j++)
        {
            ...
            {
                numerator = ((array_file_1_blue[i][angle] * r) + (array_file_2_blue[j][angle] * (1 - r)));
                denominator = (array_file_1_red[i][angle-1] * r) + (array_file_2_red[j][angle-1] * (1 - r));
                if (denominator!=0)
                {
